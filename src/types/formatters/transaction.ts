...
function formatUpdateTransaction(rpcObj: UpdateTransactionRpc): UpdateTransaction {
  return {
    ...
    userAcct: rpcObj.userAcct,
    userProof: rpcObj.userProof,
    sponsorAcct: rpcObj.sponsorAcctBytes == '0x' ? undefined : KeystoreAccountBuilder.rlpDecode(rpcObj.sponsorAcctBytes),
...
export function formatL2Transaction(rpcObj: L2TransactionRpc): L2Transaction {
  switch (rpcObj.type) {
    case TransactionType.Deposit:
      return { type: TransactionType.Deposit, ...formatDepositTransaction(rpcObj) };
    case TransactionType.Withdraw:
      return { type: TransactionType.Withdraw, ...formatWithdrawTransaction(rpcObj) };
    case TransactionType.Update:
      return { type: TransactionType.Update, ...formatUpdateTransaction(rpcObj) };
  }
}